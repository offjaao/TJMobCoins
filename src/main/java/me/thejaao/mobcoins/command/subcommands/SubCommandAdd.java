...
public class SubCommandAdd extends IArgument {

    public SubCommandAdd(String name, String... alias) {
        super(name, alias);
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length != 3) {
            sender.sendMessage("§cUtilize: /mobcoins adicionar <player> <quantidade>.");
            return;
        }
        ...
        if (isDouble(args[2])) {
            USER_MANAGER.addMobCoins(target.getName(), Double.parseDouble(args[2]));
            sender.sendMessage("§aForam adicionados §d" + BalanceHelper.prefix(Double.parseDouble(args[2])) + "§a de mobcoin para §d" + target.getName() + "§a.");
